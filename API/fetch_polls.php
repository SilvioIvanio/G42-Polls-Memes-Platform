...
$stmt = $pdo->query('SELECT p.id AS poll_id, p.user_id, p.question, p.created_at, u.username FROM polls p JOIN users u ON p.user_id = u.id ORDER BY p.created_at DESC LIMIT 50');
$polls = $stmt->fetchAll();
